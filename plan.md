...
Where O19 looks first: `src/lib/rate-limit.ts` (`LIMITS` is the whole policy; add a row rather than a number in a route), `docs/runbook.md` for what `/api/health` means, and the `PUBLIC_CSP` comment in `next.config.ts` before touching rendering — the nonce question and the per-request `<html lang>` problem in `docs/improvement-report.md` §1.11 are the same problem. `src/middleware.ts` now strips `x-site`, so anything that moves brand resolution earlier must keep that strip.
