...
export function PhotoHero({ image, eyebrow, title, sub, actions, position = 'left' }: {
  ...
  return (
    <section className={`relative isolate flex min-h-[88svh] overflow-hidden bg-black text-white md:min-h-[80vh] ${position === 'upper-left' ? 'items-start' : 'items-center'}`}>
      <img src={`/images/arrival/${image}-2400.webp`} srcSet={`/images/arrival/${image}-1200.webp 1200w, /images/arrival/${image}-2400.webp 2400w`} sizes="100vw" width={2400} height={1350} alt={asset.alt_en} loading="eager" fetchPriority="high" className="absolute inset-0 -z-20 h-full w-full object-cover" />
      {/* At least 60% black keeps white text above AA even over white pixels. */}
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(90deg,rgba(0,0,0,.82),rgba(0,0,0,.60))]" />
