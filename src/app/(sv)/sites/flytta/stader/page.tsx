...
export default function Page() {
  const posts = getHub('flytta', 'stader').filter((post) => !post.frontmatter.draft);
  return (
    <Section>
      <Container>
        ...
        <div className="mt-[var(--space-10)] grid gap-[var(--space-6)] sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((post) => (
            <Card key={post.slugPath} title={post.frontmatter.title} href={contentHref('flytta', post.slugPath)}
