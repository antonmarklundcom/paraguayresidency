...
/** Best-effort events. Callers must pass only non-identifying context. */
export function track(name: string, props?: Record<string, string>): void {
  if (process.env.NEXT_PUBLIC_PLAUSIBLE_ENABLED !== 'true' || typeof window === 'undefined') return;
  try {
    ...
    // Analytics must never interrupt a conversion, even if the script fails.
  }
}
