# Phase S14 — Guide member area + Insider sales page. Paste into a fresh SONNET session, ONLY after phase S16 (the domain sweep) is merged. S6 may still be open — it is owner-blocked and does not gate you (plan §4.12, amended F9). Runs in parallel with S10–S13.
...
Quality bar:
- Voice from §11.3 (the brand is "Paraguay Residency Guide" on `paraguayresidencyguide.com` since F9; S16 already renamed it — do not reintroduce "Investor Guide"), warm light theme from S5. `/insider` is a long-form sales page: what changes monthly, updates-feed preview, resources, price read from `products` (never typed), LS checkout button, FAQ, refund policy. "Coming soon" state when the product is inactive.
...
Phase rules:
- Branch `phase/s14` off latest main (S16 merged). Re-runnable. Minor issues → `KNOWN-ISSUES.md`; stop only per §4.4. Never hardcode a legal or financial figure (§4.11).
