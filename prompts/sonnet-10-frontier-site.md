# Phase S10 — paraguayfrontier.com pages, copy, SEO. Paste into a fresh SONNET session, ONLY after phase S6 is merged. Runs in parallel with S11–S14.
...
- `/routes` links to the hub's service pages and the Investor Pass sibling rather than restating them; `/guide` is a bridge, not a copy.
...
Phase rules:
- Branch `phase/s10` off latest main (S6 merged). Re-runnable: continue from the first unmet exit criterion.
