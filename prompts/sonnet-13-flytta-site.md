# Phase S13 — flyttatillparaguay.se port + pages (Swedish). Paste into a fresh SONNET session, ONLY after phase S6 is merged. Runs in parallel with S10–S12, S14.
...
- Personal-story voice; first person plural allowed here only. Guide upsell allowed.
...
Phase rules:
- Branch `phase/s13` off latest main (S6 merged). Re-runnable. Minor issues → `KNOWN-ISSUES.md`; stop only per §4.4. Never hardcode a legal or financial figure (§4.11).
